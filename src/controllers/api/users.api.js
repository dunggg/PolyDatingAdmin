  ...
  try {
    ...
    if (checkRemove === 'true') {
      if (images.length <= 2)
        return res
          .status(400)
          .json(response(400, 'Không thể xóa khi còn 2 ảnh'));
...
exports.updateStatusHobby = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(response(200, 'Cập nhật tìm kiếm sở thích thành công'));
  } catch (error) {
    res.status(500).json(response(500, error.message));
  }
};
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    if (!data)
      return res.status(404).json(response(404, 'Email không tồn tại'));
